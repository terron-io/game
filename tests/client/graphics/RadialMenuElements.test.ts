...
describe("RadialMenuElements", () => {
  ...
  beforeEach(() => {
    mockPlayer = {
      ...
      // terron авиация: меню проверяет units(Airport) для десант-пункта; пусто =
      // без авиации, тесты остаются про базовые атака/стройка. terron ульты:
      // подменю ещё содержат звезду-ульту (id "ultimate") — тесты её отфильтровывают.
      units: vi.fn(() => []),
      hasUltimate: vi.fn(() => false),
      // terron ульты: стройка выбирает звезду-чузер, пока выбор не зафиксирован;
      // после фиксации показывается прямой слайс выбранной ульты (ultRadialItem).
      ultimateChoice: vi.fn(() => null),
      ultStats: vi.fn(() => ({
        stolen: 0,
        stolenGained: 0,
        mirvLaunches: 0,
  ...
  describe("attackMenuElement", () => {
    ...
    it("should return attack submenu with attack units only", () => {
      ...
      // terron: подменю содержит и звезду-ульту (id "ultimate") — берём только
      // юнит-пункты (id "attack_*").
      const returnedUnitTypes = subMenu
        .filter((item) => item.id.startsWith("attack_"))
        .map((item) => {
      ...
      expect(returnedUnitTypes.length).toBeGreaterThan(0);
  ...
  describe("buildMenuElement", () => {
    ...
    it("should return build submenu with construction units only", () => {
      ...
      const constructionUnitTypes = [UnitType.City, UnitType.Factory];
      // terron: подменю содержит и звезду-ульту (id "ultimate") — берём только
      // юнит-пункты (id "build_*").
      const returnedUnitTypes = subMenu
        .filter((item) => item.id.startsWith("build_"))
        .map((item) => {
