...
function fullMessage(
  ...
  return JSON.stringify({
    type: "full",
    serverTime,
    // terron: ключи record'а = ВСЕ типы публичных лобби (zod требует их все),
    // включая golden — золотой матч, см. TerronTuning TERRON_GOLDEN_*.
    games: { ffa: [], team: [], special: [], golden: [], diamond: [], ...games },
  });
}
