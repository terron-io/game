...
export class InputHandler {
  ...
  // terron: любой юнит выбран (single/multi) → крестик-курсор легитимен. Нужен,
  // чтобы отличать «залипший от ⌘⇧-скриншота Shift» от реального выбора при
  // сбросе застрявшего крестика в onPointerMove.
  ...
  initialize() {
    ...
    // terron 20.07: колесо слушаем НА ОКНЕ, а не только на слое игры. Замер на
    // машине владельца показал: клики до слоя доходят («clicked cell» в логе), а
    // wheel — нет ни одного, поэтому камера не зумилась до старта матча. Окно
    // получает событие всегда; чужие прокручиваемые панели (чат, модалки) не
    // задеваем — если цель внутри интерактивного UI, пропускаем.
    // Колесо слушает ТОЛЬКО слой игры. 20.07 я пробовал вешать его ещё и на
    // window (гипотеза «событие не доходит до слоя») — гипотеза оказалась
    // неверной: камера была жива, замирал весь главный поток (см. CamDiag.ts).
    // Слушатель на window вернули обратно: он звал preventDefault на прокрутку
    // ВСЕЙ страницы, из-за чего колесо внутри прокручиваемых панелей HUD (чат,
    // списки) зумило бы карту вместо скролла списка.
    ...
    window.addEventListener("keyup", (e) => {
      ...
      if (this.canUseBuildKeybinds()) {
        ...
        // terron: хоткей чузера ультимейта — клавиша СЛЕВА от цифр (§/ё/~). На
        // ANSI это Backquote, на Mac-ISO — IntlBackslash (§±). Матчим ОБА кода
        // напрямую (keybindMatchesEvent ловил бы только один) + настраиваемый бинд.
        if (
          e.code === "Backquote" ||
          e.code === "IntlBackslash" ||
          this.keybindMatchesEvent(e, this.keybinds.openUltimate)
        ) {
          e.preventDefault();
          window.dispatchEvent(
            new CustomEvent("terron-toggle-ultimate-chooser"),
          );
        }
      }
  ...
  private onPointerDown(event: PointerEvent) {
    // terron: чем реально играют — палец или мышь (значок в спидран-топе).
    // Считаем ДО всех ранних выходов: средняя кнопка и ПКМ — тоже мышь.
    recordPointerInput(event.pointerType);
  ...
  /**
   * terron (13.07, репорт Тома): состояние клавиши-модификатора для КЛИКОВ
   * берём из самого события мыши (ОС всегда знает живое состояние), а не из
   * activeKeys. Системные хоткеи мака (⌘⇧5 запись экрана, ⌘⇧4 скриншот,
   * ⌘Space Spotlight) — это оверлеи: они съедают keyup БЕЗ blur окна, ⌘
   * «залипал» в activeKeys навсегда → каждый клик открывал меню стройки.
   * Заодно чистим залипший код из Set — лечит и остальных его читателей.
   */
  private modifierDown(
    code: string | undefined,
    ev: MouseEvent | PointerEvent,
  ): boolean {
    // Бинд может отсутствовать: юзер снял его («Null» удаляется из keybinds()
    // в UserSettings) или кейбинды ещё не загружены — модификатор не зажат.
    if (!code) return false;
    let live: boolean | null = null;
    if (code.startsWith("Meta")) live = ev.metaKey;
    else if (code.startsWith("Control")) live = ev.ctrlKey;
    else if (code.startsWith("Alt")) live = ev.altKey;
    else if (code.startsWith("Shift")) live = ev.shiftKey;
    if (live === null) return this.activeKeys.has(code); // небраузерный бинд
    if (!live) this.activeKeys.delete(code);
    return live;
  }
  ...
  onPointerUp(event: PointerEvent) {
    ...
    // terron: меню по клавише-модификатору (build/emoji) — это ДЕСКТОПНАЯ фича
    // (на маке modifier = ⌘). На малом вьюпорте (телефоны) клавиш нет, а в
    // мобильной эмуляции ⌘+клик ложно открывал бы сетку стройки — глушим.
    const smallViewport = window.innerWidth < 768;
    if (
      !smallViewport &&
      this.modifierDown(this.keybinds.buildMenuModifier, event)
    ) {
      this.suppressNextTap = false;
      this.eventBus.emit(new ShowBuildMenuEvent(event.clientX, event.clientY));
      return;
    }
    // terron: Alt+клик (меню эмодзи) отключён по запросу.
    // if (
    //   !smallViewport &&
    ...
    if (dist < this.DRAG_THRESHOLD_PX) {
      if (event.pointerType === "touch") {
        ...
        // На тач тап ВСЕГДА идёт обычным путём (→ радиал-меню действий). НЕ
        // перехватываем под постановку «госта»: на мобиле гост нечем отменить,
        // и при застрявшем госте все тапы хайджекались → меню не открывалось.
        // Стройка на мобиле идёт через радиал, гост-постановка — десктопная.
  ...
  // terron 20.07: страж заморозки старта (см. CamDiag.ts). Пока не пошли ходы,
  // копим длинные задачи главного потока и печатаем ОДНУ строку итога. Нужен
  // потому, что жалоба «карта видна, но не зумится и не таскается» оказалась
  // не про ввод, а про то, что поток занят и не обрабатывает события вообще.
  private diagInterval: NodeJS.Timeout | null = null;
  private stopLongTaskWatch: (() => void) | null = null;

  private startPreTickDiagnostics() {
    // Флаг уже выставлен в createClientGame (там же сбрасываются счётчики) —
    // сборка графики идёт до нас и должна попасть в разбор.
    if (!camDiag.enabled) return;
    this.stopLongTaskWatch = camDiag.watchLongTasks();
    this.diagInterval = setInterval(() => {
      if (this.gameView.ticks() === 0) return;
      console.log(
        `[cam-diag] старт матча — главный поток: ${camDiag.summary()}` +
          `\n[cam-diag] шаги старта: ${camDiag.stepsSummary()}`,
      );
      this.stopPreTickDiagnostics();
    }, 1000);
  }

  private stopPreTickDiagnostics() {
    if (this.diagInterval !== null) {
      clearInterval(this.diagInterval);
      this.diagInterval = null;
    }
    this.stopLongTaskWatch?.();
    this.stopLongTaskWatch = null;
    camDiag.enabled = false;
  }
  ...
  private onPointerMove(event: PointerEvent) {
    ...
    // terron: живое состояние Shift берём из САМОГО события мыши (ОС всегда знает),
    // а не из activeKeys — иначе ⌘⇧4/⌘⇧5-скриншот мака (и др. систем-оверлеи) съедают
    // keyup Shift БЕЗ blur окна, Shift «залипает» в Set → крестик-курсор + режим
    // бокс-селекта НАВСЕГДА → клики с микро-драгом хайджекались в выбор кораблей,
    // стройка отваливалась (репорт: «строиться нельзя», курсор +). modifierDown
    // заодно вычищает залипший код из activeKeys (лечит и камеро-пан на 313).
    const liveShift = this.modifierDown(this.keybinds.shiftKey, event);
    // Сброс ЗАЛИПШЕГО крестика: Shift реально отпущен, юнит не выбран, бокса нет.
    if (
      !liveShift &&
      !this.selectionBoxActive &&
    ...
    if (this.pointers.size === 1) {
      ...
      if (this.selectionBoxActive || liveShift || this.longPressActive) {
      ...
      } else if (this.uiState.ghostStructure === UnitType.CityTransfer) {
        // terron 25.08: ШАГАЮЩИЙ ГОРОД — drag-n-drop зданий. Пока каст взведён,
        // тянущий жест ПЕРЕТАСКИВАЕТ здание, а не двигает камеру: иначе схватить
        // здание и увезти его невозможно — карта уезжает вместе с курсором.
        // Тот же приём, что у рамки выделения кораблей выше.
