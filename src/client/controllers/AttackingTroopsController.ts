...
export class AttackingTroopsController implements Controller {
  ...
  tick() {
    ...
    for (const attack of myPlayer.incomingAttacks()) {
      // terron: битая запись атаки (attackerID undefined) не должна ронять
      // рендер-цикл КАЖДЫЙ тик — playerBySmallID(undefined) кидается (краш-луп
      // «small id undefined not found» у игроков 17.07, client_health).
      if (attack.attackerID === undefined) continue;
