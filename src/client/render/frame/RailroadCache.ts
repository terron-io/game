...
export class RailroadCache {
  ...
  /**
   * terron 25.08: МГНОВЕННАЯ УКЛАДКА (просьба владельца «сделай моментальные
   * рельсы, пока у меня эта ульта»). У Шагающего города станции перецепляются
   * на КАЖДОМ шаге здания, а прокладка анимируется ~3 тайла/тик с двух концов —
   * длинная ветка не успевала достроиться до следующей перецепки, и рельсы
   * вечно «рисовались». Под ультой рельсы появляются целиком сразу.
   * Чисто отрисовка: набор тайлов и симуляция не меняются.
   */
  ...
  private addRailroad(
    ...
    const complete = completeArg || this.instant;
