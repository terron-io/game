...
void main() {
  ...
  // terron: поезда должны читаться на любой территории. Тело (светлая полоса)
  // остаётся цветом владельца, но ОБВОДКУ (тёмная полоса) делаем контрастной к телу:
  // светлое тело → почти чёрная обводка, тёмное → почти белая. Так у поезда всегда
  // есть видимый контур, и хоть тело, хоть обводка контрастируют с фоном.
  if (vAtlasCol >= float(TRAIN_MIN_COL)) {
    float bodyLum = dot(territoryColor, vec3(0.299, 0.587, 0.114));
    borderColor = bodyLum > 0.5 ? vec3(0.06) : vec3(0.97);
  }
