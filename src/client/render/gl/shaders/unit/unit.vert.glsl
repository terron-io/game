...
// terron: авиация — поворот спрайта по направлению полёта (uint8 нормализован → [0,1] → ·2π).
// 0 = нос вверх (как рисуется в атласе). Считается на клиенте из lastPos→pos.
...
void main() {
  ...
  // terron: авиация — поворот спрайта по курсу. Вращаем ГЕОМЕТРИЮ квада (не UV), поэтому
  // sprite-текстура крутится в мире, а sample по vCellUV=aPos остаётся штатным.
  vec2 off = aPos - 0.5;
  float ca = cos(aAngle * 6.2831853);
  float sa = sin(aAngle * 6.2831853);
  vec2 roff = vec2(off.x * ca - off.y * sa, off.x * sa + off.y * ca);
  vec2 worldPos = center + roff * halfSize * 2.0;
