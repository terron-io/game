...
export class SAMRadiusPass {
  ...
  // terron: ховер-круг радиуса структуры (щит/ПВО/минправды под курсором).
  // Кладётся ПЕРВОЙ инстанцией буфера — draw() может нарисовать только её,
  // когда общая видимость (ghost/highlight) выключена.
  private hoverCircle: {
    x: number;
    y: number;
    radius: number;
  ...
  // terron: ПИРАТСТВО — зоны блокады торговли: красные круги, видны ВСЕГДА
  // (кладутся в начало буфера вслед за ховер-кругом).
  private hazardCircles: { x: number; y: number; radius: number }[] = [];
  ...
  setAllies(allies: Set<number>): void {
    // terron: зовётся КАЖДЫЙ тик из Renderer.updateNames, а rebuild = O(SAM²)
    // дуги + bufferSubData. Сет меняется редко — сравниваем и выходим (как
    // FogPass.setFriendly).
    if (allies.size === this.allies.size) {
      let same = true;
      for (const id of allies) {
        if (!this.allies.has(id)) {
          same = false;
          break;
        }
      }
  ...
  updateStructures(structures: Map<number, UnitState>): void {
    ...
    // terron: зоны блокады — первыми (group -2, ни с чем не мержатся).
    for (const hz of this.hazardCircles) {
      circles.push({
        x: hz.x,
        y: hz.y,
        radius: hz.radius,
    ...
    // terron: ховер-круг — первым (group -1 не мержится ни с кем → полная дуга,
    // ровно одна инстанция в слоте 0).
    if (this.hoverCircle) {
      circles.push({
        x: this.hoverCircle.x,
        y: this.hoverCircle.y,
        radius: this.hoverCircle.radius,
        color: this.hoverCircle.friendly ? COLOR_SELF : COLOR_ENEMY,
        group: -1,
      });
    }
    for (const u of structures.values()) {
      // terron: «Небо наше» (реворк 21.08) — штаб сам ПВО с радиусом ×5,
      // рисуем его круг вместе с обычными ПВО.
      const isOurSky = u.unitType === UT_OUR_SKY;
      if (u.unitType !== UT_SAM_LAUNCHER && !isOurSky) continue;
      ...
      circles.push({
        x,
        y,
        radius: samRange(u.level) * (isOurSky ? OURSKY_SAM_RADIUS_MULT : 1),
        color,
        group,
      });
    }
  ...
  // terron: ПИРАТСТВО — зоны блокады (всегда видимые красные круги).
  setHazardCircles(list: { x: number; y: number; radius: number }[]): void {
    this.hazardCircles = list;
    this.updateStructures(this.lastStructures ?? new Map());
  }

  // terron: ховер-круг радиуса структуры под курсором (null = убрать).
  setHoverCircle(
  ...
  draw(cameraMatrix: Float32Array): void {
    // Полная видимость (ghost/highlight) — все круги; иначе, если есть
    // ховер-круг, — только он (слот 0).
    // terron: блокады — всегда (первые N инстанций), ховер — следом.
    const always =
      this.hazardCircles.length + (this.hoverCircle !== null ? 1 : 0);
    const drawCount = this.visible
      ? this.instanceCount
      : Math.min(always, this.instanceCount);
