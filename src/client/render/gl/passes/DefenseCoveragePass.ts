...
// terron: per-instance радиус — у владельца Укреплений бункер кроет дальше
// (растёт с уровнем штаба). Раньше был один uniform на всех.
...
export class DefenseCoveragePass {
  ...
  constructor(
    ...
    // terron ПЕРФ (память, 12.07): 1×1-заглушка до ПЕРВОГО бункера. Читатели
    // (border-stamp/territory) сэмплят нулевую 1×1 = «нет защиты» — корректно.
    // Полный размер — ensureFullSize() в updateDefensePosts.
  ...
  // terron ПЕРФ (память): coverageTex развёрнута в полный размер?
  private fullSize = false;

  /** Развернуть coverage-текстуру (идемпотентно; первый бункер на карте). */
  ...
  updateDefensePosts(
    ...
    if (posts.length > 0) this.ensureFullSize();
   ...
   */
  draw(): void {
    if (!this.fullSize) return; // бункеров ещё не было — текстура 1×1 спит
    ...
    // terron: радиус теперь per-instance (aPost.w) — форт-владельцы кроют дальше.
