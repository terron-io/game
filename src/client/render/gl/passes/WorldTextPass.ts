...
/**
 * terron: авиация — обратный отсчёт иммунитета десантного плацдарма. Чуть мельче
 * атакующих подписей, но читаемо; постоянный экранный размер (делим на zoom).
 */
const BEACHHEAD_LABEL_SCREEN_SCALE = 12.0;
const BEACHHEAD_LABEL_OUTLINE_WIDTH = 1.4;
...
export class WorldTextPass {
  ...
  // terron: авиация — постоянные подписи-таймеры десантных плацдармов (отсчёт иммунитета).
  private beachheadLabels: AttackTroopLabel[] = [];
  ...
  applyConquestEvents(events: ConquestFx[]): void {
    ...
    for (const evt of events) {
      ...
      // terron: доля за «шаринг» — золотым, добивание — белым.
      this.active.push({
        x: evt.x,
        ...
        riseSpeed: 0,
        colorR: 1,
        colorG: evt.share ? 0.82 : 1,
        colorB: evt.share ? 0.28 : 1,
  ...
  /** terron: авиация — заменить набор подписей-таймеров плацдармов. */
  setBeachheadLabels(labels: AttackTroopLabel[]): void {
    this.beachheadLabels = labels;
  }
  ...
  private rebuildInstances(now: number, zoom: number): void {
    ...
    // terron: авиация — таймеры плацдармов. Постоянные, без анимации; экранный размер
    // держим постоянным (делим на zoom), как у атакующих подписей.
    const beachheadScale =
      (BEACHHEAD_LABEL_SCREEN_SCALE * dpr) / Math.max(zoom, 0.0001);
    for (const label of this.beachheadLabels) {
      layoutString(
        label.text,
        this.glyph,
        this.kernTable,
      ...
      for (let i = 0; i < len; i++) {
        ...
        this.instanceData[off + 6] = label.colorG;
        this.instanceData[off + 7] = label.colorB;
        this.instanceData[off + 8] = beachheadScale;
        this.instanceData[off + 9] = BEACHHEAD_LABEL_OUTLINE_WIDTH;
        count++;
      }
    }
