...
export class BarPass {
  ...
  private friendlyOwners: ReadonlySet<number> = new Set();
  ...
  updateBars(
    ...
    for (const unit of mobileUnits.values()) {
      // Невидимые врагу лодки (подлодка 2 / тихий десант 4) и лодки блокады
      // сверх DRAW_MAX (6) — без полоски, иначе полоска выдаёт позицию.
      if (unit.subState === 6) continue;
      if (
      ...
      // terron: ПИРАТСТВО — у пиратской лодки свой потолок (UnitState.maxHealth).
      const max = unit.maxHealth ?? this.warshipMaxHealth;
      if (unit.health === null || unit.health <= 0 || unit.health >= max)
        continue;
      this.pushHealth(unit, unit.health / max);
    }
