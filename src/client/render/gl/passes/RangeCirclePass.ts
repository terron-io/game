...
export class RangeCirclePass {
  ...
  private uMinRadius: WebGLUniformLocation;
  ...
  constructor(gl: WebGL2RenderingContext) {
    ...
    this.uMinRadius = gl.getUniformLocation(this.program, "uMinRadius")!;
  ...
  updateGhostPreview(data: GhostPreviewData | null): void {
    if (data && data.rangeRadius > 0) {
      ...
      this.minRadius = data.rangeMinRadius;
  ...
  draw(cameraMatrix: Float32Array): void {
    ...
    // minRadius > 0 → шейдер рисует градиент (мёртвая зона + green→yellow). При
    // warning принудительно красный (uMinRadius=0 отключает градиент).
    gl.uniform1f(this.uMinRadius, this.warning ? 0.0 : this.minRadius);
    if (this.warning) {
