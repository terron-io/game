 ...
 *  FX_TYPE_COUNT приватен модулю — берём отсюда, а не дублируем в реестре. */
export function fxSpriteProgramSources(): [string, string] {
  return [shaderSrc(spriteVertSrc, { FX_TYPE_COUNT }), spriteFragSrc];
}
...
export class FxSpritePass {
  ...
  constructor(
    ...
    this.program = createProgram(gl, ...fxSpriteProgramSources());
  ...
  spawnFxForUnit(unit: DeadUnitFx, now: number): void {
    ...
    // terron: авиация — дрон детонирует всегда (площадь) → мини-ядерный взрыв. Спека: airport.md
    if (typeName === UT_SUICIDE_DRONE) {
      this.spawnNukeSprites(x, y, 30, now, unit.pos);
      return;
    }
    // Десант взрывается ТОЛЬКО когда сбит ПВО (reachedTarget=false). Успешная посадка
    // (reachedTarget=true) — без взрыва. Торговые самолёты не взрываются вовсе. Спека: airport.md
    if (typeName === UT_AIRBORNE_ASSAULT && !unit.reachedTarget) {
      this.pushFx(x, y, FX_UNIT_EXPLOSION, now);
      return;
    }
