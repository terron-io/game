...
import { FxProjectilePass } from "./FxProjectilePass";
...
export class FxPass {
  ...
  private projectilePass: FxProjectilePass;
  private mapW: number;
  ...
  constructor(
    ...
    this.projectilePass = new FxProjectilePass(gl);
  }
  ...
  /** terron: «Небо наше» — пульс-кольцо тревоги на штабе в касте (маркер). */
  pushAlertPing(x: number, y: number): void {
    this.shockwavePass.pushSAMShockwave(x, y);
  }

  /** terron: Укрепления — летящий снаряд-«выстрел» бункера→захваченный тайл. */
  pushFortShot(fromX: number, fromY: number, toX: number, toY: number): void {
    this.projectilePass.push(fromX, fromY, toX, toY);
  }
  ...
  draw(cameraMatrix: Float32Array, zoom: number): void {
    ...
    this.projectilePass.draw(cameraMatrix);
  }
  ...
  setTimeFn(fn: () => number): void {
    ...
    this.projectilePass.setTimeFn(fn);
  }
