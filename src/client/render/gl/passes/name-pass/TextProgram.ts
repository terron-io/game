...
export class TextProgram {
  ...
  private uMyOwnerID: WebGLUniformLocation;
  private uCullThresholdBots: WebGLUniformLocation;
  private uCullThresholdNations: WebGLUniformLocation;
  ...
  private uFadeBigStart: WebGLUniformLocation;
  private uFadeBigEnd: WebGLUniformLocation;
  private uFadeBigMinAlpha: WebGLUniformLocation;
  // terron: фейд ника под курсором
  private uCursorWorld: WebGLUniformLocation;
  private uCursorFadeRadiusFactor: WebGLUniformLocation;
  private uCursorFadeMinAlpha: WebGLUniformLocation;
  ...
  constructor(
    ...
    // terron: локации кэшируем на старте — getUniformLocation по строке в
    // draw() каждый кадр = лишний lookup в драйвере.
    this.uCullThresholdBots = gl.getUniformLocation(
      this.program,
      "uCullThresholdBots",
    )!;
    this.uCullThresholdNations = gl.getUniformLocation(
      this.program,
      "uCullThresholdNations",
    )!;
    this.uMyOwnerID = gl.getUniformLocation(this.program, "uMyOwnerID")!;
    ...
    this.uFadeBigStart = gl.getUniformLocation(this.program, "uFadeBigStart")!;
    this.uFadeBigEnd = gl.getUniformLocation(this.program, "uFadeBigEnd")!;
    this.uFadeBigMinAlpha = gl.getUniformLocation(
      this.program,
      "uFadeBigMinAlpha",
    )!;
    this.uCursorWorld = gl.getUniformLocation(this.program, "uCursorWorld")!;
    this.uCursorFadeRadiusFactor = gl.getUniformLocation(
      this.program,
      "uCursorFadeRadiusFactor",
    )!;
    this.uCursorFadeMinAlpha = gl.getUniformLocation(
      this.program,
      "uCursorFadeMinAlpha",
    )!;
  ...
  draw(
    ...
    myOwnerID: number,
    // terron: фейд ника под курсором (мировые коорд.); factor/minAlpha из NamePass
    cursorWorldX: number,
    cursorWorldY: number,
    cursorFadeRadiusFactor: number,
    cursorFadeMinAlpha: number,
  ): void {
    ...
    gl.uniform1f(this.uCullThresholdBots, ns.cullThresholdBots);
    gl.uniform1f(this.uCullThresholdNations, ns.cullThresholdNations);
    ...
    gl.uniform1f(this.uMyOwnerID, myOwnerID);
    ...
    gl.uniform1f(this.uFadeBigStart, ns.fadeBigStart);
    gl.uniform1f(this.uFadeBigEnd, ns.fadeBigEnd);
    gl.uniform1f(this.uFadeBigMinAlpha, ns.fadeBigMinAlpha);
    gl.uniform2f(this.uCursorWorld, cursorWorldX, cursorWorldY);
    gl.uniform1f(this.uCursorFadeRadiusFactor, cursorFadeRadiusFactor);
    gl.uniform1f(this.uCursorFadeMinAlpha, cursorFadeMinAlpha);
