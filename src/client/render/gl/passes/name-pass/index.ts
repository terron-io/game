...
export class NamePass {
  ...
  // terron: Закрытая страна — smallID игроков, чья подпись войск скрыта от нас.
  private troopsHidden: ReadonlySet<number> = new Set();

  /** terron: Закрытая страна — кому НЕ рисовать цифру войск под ником. */
  setTroopsHidden(smallIDs: ReadonlySet<number>): void {
    this.troopsHidden = smallIDs;
  }

  // Hovered player's small ID (0 = no highlight, matches TerritoryPass).
  private highlightOwnerID = 0;
  // terron: свой игрок (smallID) — его ник виден всегда. 0 = своего нет.
  private myOwnerID = 0;
  private playerStateByID = new Map<string, PlayerState>();

  // terron: фейд ника под курсором — мировая позиция курсора (по умолчанию далеко,
  // чтобы ничего не тускнело до первого движения мыши). Радиус = factor × высота
  // ника (крупный ник = широкая зона наведения); minAlpha — прозрачность под мышью.
  private cursorWorldX = -1e9;
  private cursorWorldY = -1e9;
  // Радиус фейда = factor × высота ника. Ник ШИРЕ, чем высок (якорь — центр),
  // поэтому factor щедрый (~3), чтобы наведение на любую часть имени его тускнило.
  private readonly cursorFadeRadiusFactor = 3.0; // КРУТИТЬ: шире/уже зона
  private readonly cursorFadeMinAlpha = 0.22; // КРУТИТЬ: насколько прозрачно (выше = менее)
  ...
  constructor(
    ...
    this.kernTable = buildKernTable(atlas);
  ...
  /**
   * terron: поздняя установка флага игрока (клиент-резолвнутые клан-флаги —
   * сервер отдаёт их сырыми `clan:tag`, резолвит клиент async). Обновляем и
   * PlayerStatic (для будущего создания слота), и уже созданный слот +
   * перезапрашиваем текстуру (async-механизм onLayerReady обновит flagLayerIdx).
   */
  setPlayerFlag(smallID: number, url: string): void {
    if (!url) return;
    const pid = this.smallIDToPlayerID.get(smallID);
    if (pid === undefined) return;
    const ps = this.playerByID.get(pid);
    if (ps) ps.flag = url;
    const slot = this.slots.get(pid);
    if (slot && slot.flagUrl !== url) {
      slot.flagUrl = url;
      slot.flagLayerIdx = -1;
  ...
  updateNames(
    ...
    for (const [playerID, entry] of names) {
      ...
      if (slot.nameLen === 0) {
        // terron: имя рисуется родными глифами (DejaVu-атлас с кириллицей/юникодом);
        // layoutString сам маппит код-поинты → компактные индексы, неизвестное → □.
      ...
      const psHide = playerStateByID.get(playerID);
      // terron: Закрытая страна — вместо цифры «???».
      const troopStr =
        psHide !== undefined && this.troopsHidden.has(psHide.smallID)
          ? "???"
          : formatTroops(troops);
  ...
  private writePlayerDataRow(slot: PlayerSlot): void {
    ...
    // Column 4: flagLayerIdx, emojiAtlasIdx, smallID, isNation
    d[off + 16] = slot.flagLayerIdx;
    d[off + 17] = slot.emojiAtlasIdx;
    d[off + 18] = slot.static.smallID;
    // terron: НАЦИЯ (Франция, Россия…) — не то же, что племя-бот: у наций свой,
    // более мягкий порог отсечения ника (см. name.vert.glsl).
    d[off + 19] =
      slot.static.playerType === PlayerTypeEnum.Nation ? 1.0 : 0.0;
  ...
  // terron: чей ник не отсекать по зуму (свой). Ставит MyNameController.
  setMyOwner(ownerID: number): void {
    this.myOwnerID = ownerID;
  }

  // terron: позиция курсора (мир) для фейда ника под мышью. Далеко = не тускнеет.
  ...
  draw(cameraMatrix: Float32Array, ambient: number): void {
    ...
    this.textProgram.draw(
      ...
      this.cursorWorldX,
      this.cursorWorldY,
      this.cursorFadeRadiusFactor,
      this.cursorFadeMinAlpha,
    );
    ...
    this.iconProgram.draw(cameraMatrix, this.settings, this.vao, this.myOwnerID);
