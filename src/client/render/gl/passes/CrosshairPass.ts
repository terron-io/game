...
export class CrosshairPass {
  ...
  updateGhostPreview(data: GhostPreviewData | null): void {
    // terron 23.08: КТО ПОЛУЧАЕТ КРЕСТИК, решает контроллер (флаг crosshair).
    // Здесь стоял ручной список из двух типов — и каждый новый каст молча
    // оставался без всякого госта.
    if (data && data.crosshair === true) {
      this.active = true;
