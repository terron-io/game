...
export interface ModalConfig {
  ...
  maxWidth?: string;
  /** Лист на весь вьюпорт (terron 24.08, /ults). */
  fullscreen?: boolean;
}
...
export abstract class BaseModal extends LitElement {
  ...
  // terron: реестр открытых модалок — гарантия «одна за раз» (см. open/close).
  private static openModals = new Set<BaseModal>();
  ...
  /**
   * terron 25.08: гард разворачивания (см. restore()). `window.showPage()`
   * сам зовёт `.open()` у целевой inline-модалки, а `open()` дёргает
   * `onOpen(undefined)` — и хост-лобби на этом СОЗДАВАЛО НОВОЕ ЛОББИ (поймано
   * вживую: вернулся из «Рейтинга» — сменился id лобби, старое осиротело).
   * Разворачиваем мы сами, поэтому на время showPage() open() выключаем.
   */
  ...
  public open(args?: Record<string, unknown>): void {
    if (this.opening || this.restoring) return;
    this.opening = true;
    try {
      const tabs = this.modalConfig().tabs ?? [];
      if (tabs.length && this.activeTab === "") {
      ...
      // terron: ОДНА модалка за раз — закрываем все остальные открытые, каким бы
      // путём они ни открылись (inline/showPage или modalEl). Дефинитивный фикс
      // «двух окон на одном экране».
      for (const m of [...BaseModal.openModals]) {
        if (m === this) continue;
        // terron 25.08: ЛИПКОЕ ЛОББИ. Окно лобби закрывать нельзя — его
        // close() выходит из лобби. Сворачиваем: игрок ушёл смотреть вики или
        // топы, а членство в лобби осталось (см. minimize()).
        if (m.prefersMinimize()) m.minimize();
        else m.close();
      }
      BaseModal.openModals.add(this);
  ...
  /**
   * terron 25.08: СВЕРНУТЬ окно, НЕ закрывая его.
   *
   * Отличие от close() принципиальное: `onClose()` НЕ вызывается, поэтому
   * состояние подписки живо. Заведено ради лобби — там close() означает
   * «покинуть лобби», и любой переход по сайту выбрасывал игрока из очереди
   * (репорт 25.08: «нажимаю command+клик, а меня выкидывает»).
   *
   * Из реестра «одна модалка за раз» и из обработчика Escape свёрнутое окно
   * убираем: пока игрок ходит по сайту, Escape относится к тому, что он
   * видит, а не к спрятанному окну.
   */
  public minimize(): void {
    if (!this.isModalOpen) return;
    BaseModal.openModals.delete(this);
    this.unregisterEscapeHandler(); // ставит isModalOpen = false
    if (this.inline) {
  ...
  /** Вернуть свёрнутое окно на экран. Парная к minimize(); onOpen() не зовём. */
  public restore(): void {
    if (this.isModalOpen) return;
    this.registerEscapeHandler(); // ставит isModalOpen = true
    BaseModal.openModals.add(this);
    if (this.inline) {
      this.classList.remove("hidden");
      this.classList.add("block");
      this.style.pointerEvents = "auto";
      // showPage прячет остальные разделы. Наше окно он попробует открыть —
      // это гасит `restoring` (иначе onOpen() пересоздал бы лобби).
      this.restoring = true;
      try {
        window.showPage?.(this.id || this.tagName.toLowerCase());
      } finally {
  ...
  /**
   * «Меня нельзя закрывать — только сворачивать». Спрашивают навигация и
   * реестр «одна модалка за раз». По умолчанию false: обычная модалка при
   * уходе со страницы закрывается, как и раньше.
   */
  public prefersMinimize(): boolean {
    return false;
  }

  /** Хук: окно свернули (не закрыли). */
  protected onMinimize(): void {}

  /** Хук: свёрнутое окно вернули на экран. */
  ...
  public close(args?: Record<string, unknown>): void {
    ...
    if (this.inline) {
      // terron: ПРИНУДИТЕЛЬНО прячем элемент. Раньше close() только снимал
      // pointerEvents, но не добавлял .hidden → модалка оставалась видимой
      // (контент рисовался), отсюда «два окна на экране». Теперь display:none.
      this.classList.add("hidden");
      this.classList.remove("block");
      this.style.pointerEvents = "none";
      if (window.showPage) {
