...
export class LobbyTeamView extends LitElement {
      ...
      (client) => {
        ...
        return html`<span
          class="player-tag ${this.isCurrentPlayer(client)
            ? "current-player"
            : ""}"
          style="background:rgba(0,0,0,0.06);border:1px solid rgba(0,0,0,0.1);color:var(--t-ink,#fff)"
        >
          ${this.flagImg(client)}<span style="color:var(--t-ink,#fff)"
            >${displayName}</span
          >
          ${client.clientID === this.lobbyCreatorClientID
            ...
            : this.isCurrentPlayer(client)
              ? html`<span class="host-badge"
                  >(${translateText("host_modal.you_badge")})</span
                >`
              : this.onKickPlayer
                ? html`<button
            ...
            : repeat(
                ...
                (p) => {
                  ...
                  return html` <div
                    ...
                    style="color:var(--t-ink,#fff)"
                  >
                    <span class="truncate" style="color:var(--t-ink,#fff)"
                      >${this.flagImg(p)}${displayName}</span
                    >
                    ${p.clientID === this.lobbyCreatorClientID
                      ? html`<span class="ml-2 text-[11px] text-green-300"
                        ...
                        >`
                      : this.isCurrentPlayer(p)
                        ? html`<span class="ml-2 text-[11px] text-sky-300"
                            >(${translateText("host_modal.you_badge")})</span
                          >`
                        : this.onKickPlayer
                          ? html`<button
  ...
  // terron: кеш резолва флагов. Флаг может быть country:/flag:/clan: — их резолвит
  // resolveFlagUrl (как FlagInput), а не наивный assetUrl (тот бил часть флагов на
  // битую иконку). Резолвим асинхронно, кешируем, при готовности перерисовываем.
  @state() private flagUrls = new Map<string, string>();
  private flagLoading = new Set<string>();
  private ensureFlag(flag: string): void {
    if (this.flagUrls.has(flag) || this.flagLoading.has(flag)) return;
    this.flagLoading.add(flag);
    void resolveFlagUrl(flag)
      .then((url) => {
        // резолвер знает country:/flag:/clan:; для легаси-путей — фолбэк assetUrl.
        this.flagUrls = new Map(this.flagUrls).set(flag, url ?? assetUrl(flag));
        this.requestUpdate();
      })
      .catch(() => {
        this.flagUrls = new Map(this.flagUrls).set(flag, assetUrl(flag));
        this.requestUpdate();
      })
      .finally(() => this.flagLoading.delete(flag));
  }

  // terron: флаг перед ником → «(флаг)[тег] ник»
  private flagImg(client: ClientInfo) {
    if (!client.flag) return "";
    this.ensureFlag(client.flag);
    const url = this.flagUrls.get(client.flag);
    if (!url) return ""; // ещё резолвится — не показываем битую заглушку
    return html`<img
      src=${url}
      alt=""
      @error=${(e: Event) => {
        // не резолвился (нет ассета/флага) → скрываем, а не битая иконка.
        (e.target as HTMLImageElement).style.display = "none";
      }}
      style="display:inline-block;width:16px;height:11px;object-fit:cover;border-radius:2px;margin-right:5px;vertical-align:-1px"
    />`;
  }
