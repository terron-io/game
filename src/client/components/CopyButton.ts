...
export class CopyButton extends LitElement {
  ...
  // terron: на всю ширину блока (лобби: по ширине кнопок статуса выше),
  // иконки/текст распределяются с равными отступами.
  ...
  render() {
    ...
    return html`
      <div
        class="flex items-center bg-white/5 rounded-lg border border-white/10 flex-nowrap ${this
          .fullWidth
          ? "w-full gap-2 px-3 py-2"
          : "gap-0.5 px-2 py-1 max-w-[220px]"}"
      >
        ...
        <button
          ...
          class="font-mono font-bold text-white px-2 cursor-pointer select-none text-center truncate tracking-wider bg-transparent border-0 ${this
            .fullWidth
            ? "flex-1 text-sm"
            : "text-xs w-[92px] shrink-0"} ${disabledClass}"
          title="${translateText("common.click_to_copy")}"
          aria-label="${translateText("common.click_to_copy")}"
          ?disabled=${!canCopy}
          type="button"
