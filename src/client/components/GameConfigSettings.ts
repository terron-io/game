...
// terron: карточки отключения юнитов — С ИКОНКАМИ, порядок = панель стройки на
// десктопе (UnitDisplay) + лодка в конце. МИРВ переехал в подсекцию «Ультимейты».
export const unitOptions: {
  ...
  },
  {
    ...
    translationKey: "unit_type.sam_launcher",
    icon: assetUrl("images/SamLauncherIconWhite.svg"),
  },
  {
    type: UnitType.Warship,
    translationKey: "unit_type.warship",
    icon: assetUrl("images/BattleshipIconWhite.svg"),
  },
  {
    type: UnitType.AtomBomb,
    translationKey: "unit_type.atom_bomb",
    icon: assetUrl("images/NukeIconWhite.svg"),
  },
  {
    type: UnitType.HydrogenBomb,
    translationKey: "unit_type.hydrogen_bomb",
    icon: assetUrl("images/MushroomCloudIconWhite.svg"),
  },
  {
    type: UnitType.TransportShip,
    translationKey: "unit_type.boat",
    icon: assetUrl("images/BoatIconWhite.svg"),
  },
];

// terron: ультимейты — порядок как в чузере на десктопе (Ultimates.types).
// МИРВ — «базовый» ультимейт: при выключенном мастер-рычажке блокировать можно
// только его; остальные принудительно выключены.
// terron: список ульт-тогглов лобби ВЫВОДИТСЯ из реестра доступных ульт
// (Ultimates + UnitCatalog), а НЕ хардкодится — новая ульта появляется тут сама.
export const ultimateOptions: {
  type: UnitType;
  translationKey: string;
  icon: string;
}[] = Ultimates.types.map((t) => {
  const m = unitMeta(t);
  return {
    type: t,
...
const NON_MIRV_ULTS = ultimateOptions
  .map((o) => o.type)
...
export class GameConfigSettings extends LitElement {
  ...
  // terron спидран: лобби открыто из рейтинга — конфиг зафиксирован, чтобы
  // забег ТОЧНО зачёлся. Гасим все секции, КРОМЕ сложности: сложность — это
  // ось категорий топа, её менять можно. Разлочка — кнопкой в HostLobbyModal.
  @property({ attribute: false }) lockedExceptDifficulty = false;
  ...
  // terron: компактная карточка юнита с иконкой (мельче старых, но тыкабельна
  // пальцем: иконка 20px + подпись, p-2).
  private renderUnitCard(
    opt: { type: UnitType; translationKey: string; icon: string },
    disabledUnits: UnitType[],
    locked = false,
  ): TemplateResult {
    const isEnabled = !disabledUnits.includes(opt.type);
    if (locked) {
      return html`
        <div
          class="${DISABLED_CARD} p-2 flex flex-col items-center justify-center gap-1 min-h-[4.5rem]"
        >
          <img src=${opt.icon} class="w-5 h-5 opacity-60" alt="" />
          <span
            class="text-[10px] uppercase font-bold tracking-wide leading-tight text-white/50 text-center"
          >
            ${translateText(opt.translationKey)}
          </span>
        </div>
      `;
    }
    return html`
      <button
        class="${cardClass(
          isEnabled,
          // terron 23.08: ЕДИНАЯ ВЫСОТА. Подписи разной длины («Водородная
          // бомба» переносится на две строки) делали ряды сетки разной высоты
          // — панель выглядела рваной (репорт владельца). min-h + центровка
          // держат карточку одного размера при любой подписи.
          "p-2 flex flex-col items-center justify-center gap-1 min-h-[4.5rem]",
        )}"
        aria-pressed=${isEnabled}
        @click=${() => this.handleUnitToggle(opt.type, isEnabled)}
      >
        <img
          src=${opt.icon}
          class="w-5 h-5 ${isEnabled ? "" : "opacity-40"}"
          alt=""
        />
        <span
          class="text-[10px] uppercase font-bold tracking-wide leading-tight text-center ${stateTextClass(
            isEnabled,
          )}"
        >
          ${translateText(opt.translationKey)}
  ...
  private renderUnitTypeOptions(disabledUnits: UnitType[]): TemplateResult[] {
    return unitOptions.map((opt) => this.renderUnitCard(opt, disabledUnits));
  }

  // terron: ультимейты — мастер-рычажок + карточки. Мастер OFF = все ульты,
  // кроме «базового» МИРВ, принудительно выключены (их карточки заблокированы).
  private ultimatesMasterOn(disabledUnits: UnitType[]): boolean {
    return !NON_MIRV_ULTS.every((t) => disabledUnits.includes(t));
  }

  private handleUltimatesMaster(disabledUnits: UnitType[]): void {
    const on = this.ultimatesMasterOn(disabledUnits);
    for (const t of NON_MIRV_ULTS) {
      const disabled = disabledUnits.includes(t);
      // ON→OFF: выключаем все не-МИРВ; OFF→ON: включаем все не-МИРВ.
      if (on && !disabled) this.handleUnitToggle(t, true);
      if (!on && disabled) this.handleUnitToggle(t, false);
    }
  }

  private renderUltimatesBlock(disabledUnits: UnitType[]): TemplateResult {
    const masterOn = this.ultimatesMasterOn(disabledUnits);
    return html`
      <div class="mt-4">
        <div class="flex items-center gap-3 pb-2 border-b border-white/10">
          <h4
            class="text-sm font-bold text-white/80 uppercase tracking-wider flex-1"
          >
            ${translateText("host_modal.ultimates")}
          </h4>
          <button
            role="switch"
            aria-checked=${masterOn}
            title=${translateText("host_modal.ultimates_hint")}
            class="relative w-11 h-6 rounded-full transition-colors duration-200 ${masterOn
              ? "bg-malibu-blue/70"
              : "bg-white/15"}"
            @click=${() => this.handleUltimatesMaster(disabledUnits)}
          >
            <span
              class="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform duration-200 ${masterOn
                ? "translate-x-5"
                : ""}"
            ></span>
          </button>
        ...
        <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mt-3">
          ${ultimateOptions.map((opt) =>
            this.renderUnitCard(
              opt,
              disabledUnits,
              !masterOn && opt.type !== UnitType.MIRV,
            ),
          )}
        </div>
      </div>
  ...
  render() {
    ...
    // Обёртка «только для чтения»: секция видна, но не кликается.
    const lock = (tpl: unknown) =>
      this.lockedExceptDifficulty
        ? html`<div class="pointer-events-none opacity-50 select-none">
            ${tpl}
          </div>`
        : tpl;
        ...
        ${lock(
          renderSection(
            ...
            html`
              ...
              ${this.renderUltimatesBlock(settings.unitTypes.disabledUnits)}
            `,
            "space-y-4 pb-6",
          ),
        )}
