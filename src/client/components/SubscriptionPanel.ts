...
export class SubscriptionPanel extends LitElement {
  ...
  private handleManage = async (): Promise<void> => {
    ...
    if (url === false) {
      toast(translateText("account_modal.subscription_portal_failed"));
      return;
    }
  ...
  private handleCancel = async (): Promise<void> => {
    ...
    if (!ok) {
      toast(translateText("account_modal.cancel_subscription_failed"));
      return;
    }
    toast(translateText("account_modal.cancel_subscription_success"));
    invalidateUserMe();
