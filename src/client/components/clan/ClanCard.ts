...
export class ClanCard extends LitElement {
  ...
  // terron: флаг клана (data-URL/ссылка). Если задан — рисуем картинку вместо
  // дефолтного градиент-бокса с тегом. См. ClanCreatePage / clans.md.
  @property() flagUrl?: string;
    ...
    return html`
      ...
      >
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-xl overflow-hidden bg-gradient-to-br ${clan.isOpen
          ...
          >
            ${this.flagUrl
              ? html`<img
                ...
                />`
              : html`<span class="text-white font-bold text-sm">${clan.tag}</span>`}
          </div>
