...
export class TransformHandler {
  ...
  boundingRect(): DOMRect {
    // terron 20.07: кэш снимается ОДИН РАЗ в конструкторе — а камера создаётся
    // в окне ожидания старта, когда у страницы ещё нулевая высота. Нулевой rect
    // ломал minScale() и clampOffsets(): камера «двигалась», но ограничители
    // тут же возвращали её назад — игрок не мог ни зумить, ни таскать карту,
    // пока матч не начнётся (репорт владельца 20.07). Обновляем лениво, как
    // только видим нулевой размер; дальше работает обычный resize-хук.
    if (!this._boundingRect.width || !this._boundingRect.height) {
      this.updateCanvasBoundingRect();
    }
    // Если и после обновления нули — вёрстка ещё не разложена (окно ожидания
    // старта: у html/body высота 0). Обновлять там нечего, поэтому берём
    // размеры САМОГО ОКНА. Иначе minScale()/clampOffsets() считают по нулям и
    // возвращают камеру назад: зум и перетаскивание «не работают», пока не
    // пойдут ходы (репорт владельца 20.07, подтверждён замером на его машине —
    // после старта слой 1103x790, в ожидании вёрстка нулевая).
    if (!this._boundingRect.width || !this._boundingRect.height) {
  ...
  // terron: минимальный зум — карта должна занимать ≥70% экрана по «тесной»
  // стороне (нельзя выскроллить так, чтобы карта стала крошечной точкой).
  private minScale(): number {
    const r = this.boundingRect();
    const gw = this.game.width();
    const gh = this.game.height();
    if (!gw || !gh || !r.width || !r.height) return 0.2;
    return 0.7 * Math.min(r.width / gw, r.height / gh);
  }
  hasChanged(): boolean {
    return this.changed;
  }
  ...
  centerCamera() {
    ...
    const nl = player?.nameLocation();
    if (!nl) return;
    this.target = new Cell(nl.x, nl.y);
    this.intervalID = setInterval(() => this.goTo(), GOTO_INTERVAL_MS);
  }
  ...
  onZoom(event: ZoomEvent) {
    ...
    // Clamp the scale to prevent extreme zooming. Нижний предел динамический:
    // карта не может стать меньше ~70% экрана (иначе её не видно/не ткнуть спавн).
