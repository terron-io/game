import { releaseGlContext } from "../render/gl/GlContext";
...
export async function collectGraphicsDiagnostics(
  ...
  if (gl) {
    ...
    // ⚠️ terron 20.07: диагностический контекст отпускаем — лимит живых
    // контекстов на вкладку конечен (~16), а исчерпав его, браузер отдаёт
    // «WebGL2 not supported» уже на игровом контексте (см. releaseGlContext).
    releaseGlContext(gl);
  }
