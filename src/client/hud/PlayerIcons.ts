...
export function getPlayerIcons(
  ...
  // Crown icon for first place — но не во время фазы спавна (матч ещё не начался,
  // «лидер» по тайлам бессмысленен и корона мелькает на всех подряд).
  if (player === firstPlace && !game.inSpawnPhase()) {
