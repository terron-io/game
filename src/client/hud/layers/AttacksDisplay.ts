...
import { landsNearPlayer } from "./IncomingLanding";
...
export class AttacksDisplay extends LitElement implements Controller {
  ...
  /** id лодки → точка высадки, по которой уже принято решение (кэш круга). */
  private landingChecked: Map<number, number> = new Map();
  ...
  // плавно меняются, 250мс неощутимы; минус filter-аллокации на каждый тик).
  ...
  tick() {
    ...
    // terron 19.08: ДОСТРАИВАЕМ НАБОР ИЗ СОСТОЯНИЯ, а не только из события.
    // `displayIncomingUnit` шлётся ОДИН раз при отправке лодки и только если
    // цель лодки — владелец тайла назначения (TransportShipExecution). Отсюда
    // две дыры, обе видел владелец 19.08: (1) переподключился после отправки —
    // событие уже прошло, индикатора нет НИКОГДА, хотя десант в пути;
    // (2) бот целится в ничейный тайл у твоей границы — предупреждения не было
    // вовсе. Состояние же честно есть у всех клиентов каждый тик: активный
    // транспорт, чья точка высадки принадлежит мне, — это и есть «ко мне едут».
    const liveBoatIDs = new Set<number>();
    for (const boat of this.game.units(
      UnitType.TransportShip,
      UnitType.AirborneAssault,
    )) {
      if (!boat.isActive()) continue;
      const owner = boat.owner();
      if (owner === myPlayer || myPlayer.isFriendly(owner)) continue;
      // terron: ПИРАТСТВО — тихий десант (subState 4) в списке не светится.
      if (boat.subState() === 4) continue;
      const dst = boat.targetTile();
      if (dst === undefined) continue;
      liveBoatIDs.add(boat.id());
      // Решение по лодке не меняется, пока не сменилась точка высадки — круг
      // радиуса TERRON_INCOMING_LANDING_RADIUS считаем один раз на цель.
      if (this.landingChecked.get(boat.id()) === dst) continue;
      this.landingChecked.set(boat.id(), dst);
      if (landsNearPlayer(this.game, dst, myPlayer)) {
        this.incomingBoatIDs.add(boat.id());
      }
    }
    // Прибираем кэш за утонувшими и высадившимися.
    for (const id of this.landingChecked.keys()) {
      if (!liveBoatIDs.has(id)) this.landingChecked.delete(id);
    }
    ...
    for (const unitID of this.incomingBoatIDs) {
      ...
      // terron: авиация — десантный борт шлёт тот же NAVAL_INVASION_INBOUND, показываем
      // защитнику такой же индикатор вторжения, как у морской высадки. airport.md
      if (
        unit &&
        unit.isActive() &&
        (unit.type() === UnitType.TransportShip ||
          unit.type() === UnitType.AirborneAssault)
      ) {
