...
export class ImmunityTimer extends LitElement implements Controller {
  ...
  createRenderRoot() {
    ...
    // terron iOS: под нотч (синхрон со SpawnTimer)
  ...
  render() {
    ...
    // terron: полоса УБЫВАЕТ (100%→0%) по мере истечения перемирия — «сколько
    // осталось», как таймер. Раньше росла 0%→100% (прогресс к концу) — по просьбе
    // владельца перевёрнуто. progressRatio = доля прошедшего времени.
