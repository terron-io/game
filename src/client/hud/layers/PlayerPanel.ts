...
import { customAvatarUrl, defaultAvatar } from "../../Avatar";
import { fetchMyClans, type ClanMine } from "../../ClanApi";
...
import { accountSlugForClient } from "../../MatchAccounts";
...
export class PlayerPanel extends LitElement implements Controller {
  ...
  // terron: in-game приглашение в клан (кланы, где я ЛИДЕР).
  @state() private myLeaderClans: ClanMine[] = [];
  @state() private showInviteMenu = false;
  private clansLoaded = false;
  ...
  public show(actions: PlayerActions, tile: TileRef) {
    // terron: туман войны — досье игрока по закрытому тайлу не открываем
    // (иначе клик в черноту = разведка троп/золота).
    if (!this.g.isTileVisibleUnderFog(tile)) {
      toast(
        L(
          "Территория скрыта туманом войны",
          "This area is hidden by fog of war",
        ),
        "info",
      );
      return;
  ...
  // terron: «Пригласить в клан» — список моих кланов (где я лидер) → инвайт по
  // clientID цели (гейм-сервер резолвит persistentID). Аноним → pending до входа.
  private async toggleInviteMenu() {
    if (!this.clansLoaded) {
      const mine = (await fetchMyClans()) ?? [];
      this.myLeaderClans = mine.filter((c) => c.role === "leader");
      this.clansLoaded = true;
    }
    if (this.myLeaderClans.length === 0) {
      toast(
        L("У вас нет кланов, где вы лидер.", "You don't lead any clans."),
        "info",
      );
      return;
    }
    this.showInviteMenu = !this.showInviteMenu;
  }

  private sendClanInvite(other: PlayerView, tag: string) {
    const target = other.clientID();
    if (!target) return;
    this.eventBus.emit(new SendClanInviteIntentEvent(target, tag));
    this.showInviteMenu = false;
    toast(L("Приглашение отправлено", "Invitation sent"), "success");
  }

  // terron: заявка в друзья из большой панели (результат/уведомление придут от
  // гейм-сервера тостом + строкой в чат-ленте, как в мини-меню).
  private sendFriendRequest(other: PlayerView) {
    const target = other.clientID();
    if (!target) return;
    this.eventBus.emit(new SendFriendRequestIntentEvent(target));
    toast(L("Запрос в друзья отправлен", "Friend request sent"), "success");
  }

  private renderFriendRequest(my: PlayerView, other: PlayerView) {
    if (other.type() !== PlayerType.Human) return html``;
    if (!other.clientID() || other === my) return html``;
    return html`
      <ui-divider></ui-divider>
      <div class="grid auto-cols-fr grid-flow-col gap-1">
        ${actionButton({
          onClick: () => this.sendFriendRequest(other),
          icon: allianceIcon,
          iconAlt: "Add friend",
          title: translateText("player_panel.add_friend"),
          label: translateText("player_panel.add_friend"),
          type: "green",
        })}
      </div>
    `;
  ...
  private renderClanInvite(my: PlayerView, other: PlayerView) {
    if (other.type() !== PlayerType.Human) return html``;
    if (!other.clientID() || other === my) return html``;
    return html`
      <ui-divider></ui-divider>
      <div class="grid auto-cols-fr grid-flow-col gap-1">
        ${actionButton({
          onClick: () => void this.toggleInviteMenu(),
          icon: allianceIcon,
          iconAlt: "Invite to clan",
          title: translateText("player_panel.invite_clan"),
          label: translateText("player_panel.invite_clan"),
          type: "sky",
        })}
      </div>
      ${this.showInviteMenu
        ? html`<div class="mt-1 flex flex-col gap-1">
            ${this.myLeaderClans.map((c) => {
              const b = bracketPair(c.bracket);
              return html`<button
                class="w-full text-left px-3 py-2 rounded bg-white/10 hover:bg-white/20 text-white text-sm"
                @click=${() => this.sendClanInvite(other, c.tag)}
              >
                ${b.l}${c.tag}${b.r} ${c.name}
              </button>`;
            })}
          </div>`
        : ""}
  ...
  // terron: аватарка игрока в панели. Размер как у флага (40px) — панель узкая,
  // крупнее будет спорить с ником. Только ЛЮДЯМ: у наций/племён свой чип типа,
  // портрет там смотрелся бы как чужой аккаунт.
  //
  // Кастомную картинку берём по хэндлу аккаунта из стартовой информации матча
  // (MatchAccounts). Есть ли она у игрока, клиент в матче не знает, поэтому
  // ставим ссылку сразу, а на 404/ошибку падаем на базовый портрет по seed —
  // тот самый, что видит каждый новый игрок.
  private renderAvatar(other: PlayerView) {
    if (other.type() !== PlayerType.Human) return "";
    const seed = other.clientID() ?? other.name();
    const fallback = defaultAvatar(seed, 64);
    const slug = accountSlugForClient(other.clientID());
    const src = slug ? customAvatarUrl(slug) : fallback;
    return html`<img
      src=${src}
      alt=""
      class="h-10 w-10 rounded-lg object-cover bg-zinc-800 ring-1 ring-white/10"
      @error=${(e: Event) => {
      ...
      <div class="flex items-center gap-2.5 flex-wrap">
        ...
        ${chip
          ? html`<span
            ...
            >
              <span aria-hidden="true" class="leading-none inline-flex"
                >${uiIcon(chip.icon, 13)}</span
              >
              <span class="tracking-tight"
      ...
      <div class="mb-1 flex justify-between gap-2">
        <div
          class="inline-flex items-center gap-1.5 rounded-lg bg-white/4 px-3 py-1.5 shrink-0
                    text-white w-35"
        >
          <span class="mr-0.5 inline-flex">${uiIcon("coins", 14)}</span>
          <span translate="no" class="tabular-nums w-[5ch] font-semibold">
            ${this.g.statsHiddenFor(other)
              ? "???"
        ...
        >
          <span class="mr-0.5 inline-flex">${uiIcon("shield", 14)}</span>
          <span translate="no" class="tabular-nums w-[5ch] font-semibold">
            ${this.g.statsHiddenFor(other)
              ? "???"
  ...
  private renderActions(my: PlayerView, other: PlayerView) {
    ...
    // terron 22.08: кнопка доната союзнику-человеку в FFA не исчезает молча —
    // показываем приглушённой, клик объясняет причину (см. DonateGate).
    // Кулдаун и «не союзник» по-прежнему прячут кнопку: там объяснять нечего.
    const goldOffByMode = donateOffByMode(this.g, my, other, "gold");
    const troopsOffByMode = donateOffByMode(this.g, my, other, "troops");
      ...
      <div class="flex flex-col gap-2.5">
        <div class="grid auto-cols-fr grid-flow-col gap-1">
          ...
          ${canDonateTroops
            ...
            : troopsOffByMode
              ? html`<div class="opacity-50">
                  ${actionButton({
                    onClick: () => this.explainDonateOff(),
                    icon: donateTroopIcon,
                    iconAlt: "Troops",
                    title: donateOffText(),
                    label: translateText("player_panel.troops"),
        ...
        ${other === my
          ...
          : ""}
        ${this.renderFriendRequest(my, other)}
        ${this.renderClanInvite(my, other)}
