...
export class ControlPanel extends LitElement implements Controller {
  ...
  // terron: один инстанс — читаем дефолт в init() и персистим живые изменения
  // ползунка обратно (см. onAttackRatioChange), чтобы % атаки сохранялся между
  // играми/вкладками (localStorage-ключ settings.attackRatio).
  ...
  // terron: приток населения от Мин правды. ultStats().stolenGained — кумулятивный
  // счётчик переманенного; министерство даёт «лампой» раз в секунду, поэтому
  // усредняем прибавку по скользящему ~3с окну и вливаем в «+X/с» роста населения.
  private _ministryRate = 0;
  private _stolenHist: { t: number; v: number }[] = [];

  // Показываемый прирост населения = базовый рост + приток министерства.
  private get shownTroopRate(): number {
    return this.troopRate + this._ministryRate;
  }
  ...
  // terron perf (Р1): 5 Lit-рендеров/с вместо 10 — глаз разницы в цифрах
  // золота/войск не видит, main-поток разгружается (особенно мобила).
  ...
  tick() {
    ...
    this.updateMinistryRate(player);
      ...
      if (conquestEvents) {
        for (const ev of conquestEvents) {
          if (ev.conquerorId === myID && ev.gold > 0n) {
            ...
            // terron: моя доля за «шаринг» тоже в индикатор прироста золота
            const mine = ev.shares.find((s) => s.id === myID);
            if (mine && mine.gold > 0n) this.addGoldGain(mine.gold);
          }
        }
      }
  ...
  // Приток населения от Мин правды в войск/с (усреднение по ~3с окну кумулятивного
  // stolenGained). Нет министерства → stolenGained не растёт → rate затухает к 0.
  private updateMinistryRate(
    player: NonNullable<ReturnType<GameView["myPlayer"]>>,
  ): void {
    const stolen = player.ultStats().stolenGained;
    const now = Date.now();
    this._stolenHist.push({ t: now, v: stolen });
    const cutoff = now - 3000;
    while (this._stolenHist.length > 2 && this._stolenHist[0].t < cutoff) {
      this._stolenHist.shift();
    }
        ...
        >
          ...
          <span
            class="text-[10px] font-bold drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)] ${this
              ...
              ? "text-green-400"
              : "text-orange-400"}"
            >+${renderTroops(this.shownTroopRate)}/s</span
          >
        </div>
      </div>
    `;
  ...
  private renderDesktop() {
    return html`
      ...
      <div class="flex gap-1.5 items-center mb-1">
        ...
        <!-- Gold. terron: min-w вместо фикс-ширины — большие суммы (6751.7M) не
             вылезают за бокс/экран, растут по контенту; полоса войск (flex-1) ужимается. -->
        <div
          class="flex items-center justify-center gap-1 shrink-0 border rounded-md border-yellow-400 font-bold text-yellow-400 text-sm py-0.5 px-1 min-w-[4.5rem] relative"
          translate="no"
        >
          ${this._goldGain !== null
            ? keyed(
