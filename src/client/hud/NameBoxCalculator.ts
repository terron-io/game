...
// terron: спавн-регион теперь ПРЯМОУГОЛЬНИК-флаг (getSpawnTiles). Имя в фазе
// спавна сайзим по реальным размерам флага (а не по euclDist-диаметру 8).
const SPAWN_REGION_WIDTH = TERRON_SPAWN_RECT_HALF_W * 2 + 1;
const SPAWN_REGION_HEIGHT = TERRON_SPAWN_RECT_HALF_H * 2 + 1;
  ...
  const fontSize = calculateFontSize(
    {
      x: 0,
      y: 0,
      width: SPAWN_REGION_WIDTH,
      height: SPAWN_REGION_HEIGHT,
    },
    player.displayName(),
  );
  // terron: ник спавна — на ЦЕНТР тайла-флага (+0.5), без сдвига вверх, чтобы
  // совпадал с рамкой спавна и полем флага (раньше −fontSize/3 уводил ник вверх).
  return {
    x: game.x(spawnTile) + 0.5,
    y: game.y(spawnTile) + 0.5,
    size: fontSize,
  };
}
