...
export class UserSettingModal extends BaseModal {
  ...
  protected modalConfig() {
    // terron: в нативном приложении (Apple Guideline 4) НЕ показываем вкладку
    // переназначения клавиш — на тач-устройстве клавиатурных контролов быть не
  ...
  protected renderBody(tab: string) {
    if (tab === "account") {
      return html`<account-settings></account-settings>`;
    }
    if (tab === "petridish") {
      return html`<petridish-settings></petridish-settings>`;
    }
    const body =
      tab === "keybinds" && !isNativeApp()
  ...
  private renderKeybindSettings() {
    return html`
      <div
        class="t-info-hint flex items-center gap-2 px-3 py-2 mb-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-300/70 text-xs"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
