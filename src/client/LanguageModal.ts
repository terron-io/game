...
interface LanguageOption {
  ...
  native: string;
  en: string;
  /** terron: имя языка по-русски (metadata.json). */
  ru?: string;
}
...
export class LanguageModal extends BaseModal {
  ...
  /**
   * Вторая строка карточки — название языка НА ЯЗЫКЕ ИНТЕРФЕЙСА, а не всегда
   * английское: в русском меню «DEUTSCH / GERMAN» читалось как недоперевод
   * (замечание модерации VK 25.08.2026). Для остальных языков всё как было —
   * английский тут универсальный посредник, менять его на русский нельзя.
   */
  private secondaryName(lang: LanguageOption): string {
    const name = this.currentLang === "ru" && lang.ru ? lang.ru : lang.en;
    // «РУССКИЙ / Русский» и «ENGLISH / English» — строка-дубль, второй ряд
    // карточки в этом случае просто не нужен.
    return name.toLowerCase() === lang.native.toLowerCase() ? "" : name;
  }
