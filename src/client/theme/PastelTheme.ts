...
export class PastelTheme implements Theme {
  ...
  // terron: общий реестр цветов ЛЮДЕЙ и НАЦИЙ — нация не должна получить цвет,
  // неотличимый от игрока (репорты 17.07 «противник на моей территории»).
  // Боты сюда НЕ входят: их сотни, задавили бы подбор (кап 300 в аллокаторе).
  private sharedHumanNationColors: Colord[] = [];
  private humanColorAllocator = new ColorAllocator(
    humanColors,
    fallbackColors,
    this.sharedHumanNationColors,
  );
  ...
  private nationColorAllocator = new ColorAllocator(
    nationColors,
    nationColors,
    this.sharedHumanNationColors,
  );
