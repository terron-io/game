...
import { persistFlushSync } from "./GamePersistence";
...
export async function startWorker() {
  ...
  const wss = new WebSocketServer({
    ...
    // terron perf (E1): сжатие WS — turn'ы это повторяющийся JSON, жмётся в
    // 3-5×. Мелочь (<1КБ: пинги/интенты) не жмём (threshold). Окно 4КБ +
    // clientNoContextTakeover — экономия памяти на соединение.
    perMessageDeflate: {
      threshold: 1024,
  ...
  // terron: при рестарте (SIGTERM от docker/деплоя) — оповестить клиентов «сервер
  // перезапускается» до выхода, чтобы они показали баннер и ждали реконнекта.
  // Ходы теперь батчатся (S1) → перед выходом ОБЯЗАТЕЛЕН persistFlushSync():
  // дописывает накопленный хвост лога синхронно. См. server-reconnect.md.
  let shuttingDown = false;
  const gracefulShutdown = (sig: string) => {
    if (shuttingDown) return;
    shuttingDown = true;
    try {
      log.info(`worker ${workerId} received ${sig}, notifying clients`);
      persistFlushSync();
      gm.broadcastServerRestart();
    } catch (e) {
      log.error(`error on graceful shutdown: ${e}`);
    }
    // дать WS-фреймам уйти, затем выйти (docker всё равно SIGKILL по таймауту)
  ...
  // terron: заявка на смену карты пустого лобби едет мастеру немедленно, а не
  // со следующим бродкастом — иначе превью на витрине меняется на ~секунду
  // позже нуля на таймере. Дёшево: обычно проверка ничего не отправляет.
  startPolling(async () => lobbyService.pollRotationRequests(), 100);
  ...
  // terron: статус воркера для скрипта рестарта — сколько игр и подключённых
  // клиентов. Если клиентов 0 на всех воркерах — ребутить можно немедленно.
  app.get("/api/status", (req, res) => {
    if (process.env.API_KEY && req.get("x-api-key") !== process.env.API_KEY) {
      res.status(403).json({ error: "forbidden" });
      return;
    }
    res.json({ games: gm.activeGames(), clients: gm.activeClients() });
  });

  // terron: публичный счётчик онлайна для футера (просто число игроков воркера).
  app.get("/api/online", (_req, res) => {
    // up = аптайм game-процесса (с). Процесс рестартует только на rebuild-game,
    // поэтому это ≈ «игра обновлена N назад» для футера.
    res.json({ n: gm.activeClients(), up: Math.floor(process.uptime()) });
  });

  // terron: распределение онлайна для /stats — где сидят клиенты (матчи vs
  // лобби, публичные vs приватные). Публичный read-only.
  app.get("/api/online_breakdown", (_req, res) => {
    res.json(gm.clientsBreakdown());
  });

  // terron: витрина «Активные игры» для /stats — список ИДУЩИХ публичных матчей
  // (карта/режим/игроки/сколько идёт). Спектейт = открыть /game/<id> (сервер
  // отдаёт догоняющие ходы). Публичный read-only, без ключа.
  app.get("/api/active_games", (_req, res) => {
    res.json({ serverTime: Date.now(), games: gm.activeGamesInfo() });
  });
  ...
  app.get("/api/game/:id/exists", async (req, res) => {
    ...
    // terron 20.07: эту проверку клиент делает РОВНО перед входом (см.
    // Main.publicLobbyExists / JoinLobbyModal.checkActiveLobby) — значит сюда
    // сейчас зайдёт игрок. Замораживаем ротацию карты, иначе он кликает по
    // одной карте, а попадает на другую (ротация идёт каждые 10с).
    game?.holdRotation();
    res.json({
      exists: game !== null,
    });
  ...
  app.post("/api/archive_singleplayer_game", async (req, res) => {
    try {
      ...
      // SECURITY (C3): без auth любой curl архивировал выдуманный SP-record
      // (сервер сам ставит валидный gitCommit) и фармил валюту/ачивки на любой
      // persistentID. Требуем Bearer-токен и persistentID игрока == sub токена
      // (как create_game). Аноним/чужой pid → отказ.
      const authHeader = req.headers.authorization;
      if (!authHeader?.startsWith("Bearer ")) {
      ...
      const recordPid = gameRecord.info.players[0]?.persistentID;
      if (!recordPid || recordPid !== auth.persistentId) {
        log.warn(`SP archive persistentID mismatch`, {
          gameID: gameRecord.info.gameID,
        });
        return res.status(403).json({ error: "persistentID mismatch" });
      }
...
// terron: реальный IP игрока за цепочкой прокси. Инфра ФИКСИРОВАНА (2 доверенных
// хопа): клиент → Caddy → nginx(game:80) → этот воркер. socket.remoteAddress =
// localhost (nginx→воркер), поэтому раньше client.ip у всех был ::ffff:127.0.0.1
// (IP-гард и аудит не работали). X-Forwarded-For к воркеру приходит как
// "[поддельный клиентский...], realclient, caddy_ip": Caddy дописывает настоящего
// TCP-пира (realclient), nginx ($proxy_add_x_forwarded_for) дописывает СВОЙ пир
// (Caddy). Значит настоящий клиент — ПРЕДПОСЛЕДНИЙ элемент (length-2); всё левее
// клиент может подделать, самый правый — это Caddy. cf-connecting-ip оставлен на
// случай будущего CF-фронта. НЕ влияет на подсчёт онлайна (он по WS-коннектам).
// ⚠️ Если число прокси-хопов изменится (уберут nginx / добавят слой) — пересчитать.
function normalizeIp(ip: string): string {
  return ip.replace(/^::ffff:/i, "").trim(); // IPv4-mapped IPv6 → чистый IPv4
}
function getClientIp(req: http.IncomingMessage): string {
  ...
  if (typeof xff === "string" && xff) {
    const parts = xff
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    // предпоследний = realclient (Caddy), последний = caddy_ip (nginx). Меньше 2
    // элементов быть не должно, но фолбэк — крайний левый.
    if (parts.length >= 2) return normalizeIp(parts[parts.length - 2]);
