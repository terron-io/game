...
export class MapPlaylist {
  ...
  public async gameConfig(type: PublicGameType): Promise<GameConfig> {
    ...
    return {
      ...
      // terron: флаг СОБЫТИЙНОГО матча (награда победителю). Ставится ТОЛЬКО
      // тут и только событийным типам — у GameServer стоит гард на всё
      // остальное. eventTier говорит, какая именно награда (см. TerronTuning).
      golden: MapPlaylist.isEvent(type) ? true : undefined,
