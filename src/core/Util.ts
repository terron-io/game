...
/**
 * terron 23.08 — РАЗОБРАТЬ ГОТОВОЕ ИМЯ обратно на клан-тег и ник.
 *
 * Нужно для плашек, где длинное имя переносится: браузер рвёт его по
 * последнему влезающему пробелу, и получается «[ZBS] Galactic / Empire» —
 * тег прилипает к половине ника. Решение владельца: тег отдельной строкой,
 * ник под ним (а если и ник не влез — переносится уже он сам).
 *
 * ⚠️ Парсим строку, а не берём поля: у `PlayerView` наружу торчит только
 * собранное `displayName`, а внутри матча имя приходит с сервера уже склеенным.
 */
export function splitClanName(display: string): {
  clanTag: string | null;
  username: string;
} {
  const m = /^\[([^\]]{1,16})\]\s+(.+)$/.exec(display);
  if (m === null) return { clanTag: null, username: display };
  return { clanTag: m[1], username: m[2] };
}

/** Длина имени, после которой тег в плашке уезжает на свою строку. */
export const NAME_TWO_LINE_THRESHOLD = 16;

// terron: latin/цифры + кириллица (U+0400–U+04FF) — теги можно писать на русском.
const CLAN_TAG_CHARS = "a-zA-Z0-9Ѐ-ӿ";

const CLAN_TAG_INVALID_CHARS = new RegExp(`[^${CLAN_TAG_CHARS}]`, "g");

export function sanitizeClanTag(tag: string): string {
  // terron: НЕ форсим заглавные — сохраняем регистр как ввёл юзер.
