...
export const ClanInfoSchema = z.object({
  ...
  // terron: name/description — БАЗА (английский, фолбэк). *Ru — опциональный
  // русский оверлей: показываем на ru-локали, иначе EN-базу. См. localizeClanText.
  nameRu: z.string().max(35).optional(),
  descriptionRu: z.string().max(200).optional(),
  isOpen: z.boolean(),
  createdAt: z.iso.datetime().optional(),
  memberCount: z.number().optional(),
  slug: z.string().optional(), // terron: для навигации из браузера кланов → /@slug
});
...
export const ClanMemberSchema = z.object({
  ...
  publicId: z.string(),
  name: z.string().optional(), // terron: отображаемое имя (publicId — это slug/url)
