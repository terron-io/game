...
export class RailNetworkImpl implements RailNetwork {
  ...
  private connectToExistingRails(station: TrainStation): boolean {
    ...
    for (const rail of rails) {
      ...
      // terron: станция стоит в радиусе до stationRadius от рельса, и точка
      // разреза (closest) — НЕ тайл станции. Раньше половинки доходили только до
      // точки разреза → между станцией и сетью оставался визуальный/логический
      // разрыв в 1..stationRadius тайлов. Достраиваем коннектор разрез↔станция и
      // вшиваем его в обе половины, чтобы рельс физически доходил до станции.
      const junctionTile = rail.tiles[closestRailIndex];
      const connector = this.pathService.findTilePath(
        junctionTile,
        station.tile(),
      ); // [junction, …, station] (A* включает оба конца)
      const headTiles = rail.tiles.slice(0, closestRailIndex); // from … junction-1
      const tailTiles = rail.tiles.slice(closestRailIndex); // junction … to
      const fromTiles =
        connector.length > 0 ? headTiles.concat(connector) : headTiles;
      const toTiles =
        connector.length > 0
          ? connector.slice().reverse().concat(tailTiles.slice(1))
          : tailTiles;

      const newRailFrom = new Railroad(from, station, fromTiles, this.nextId++);
      const newRailTo = new Railroad(station, to, toTiles, this.nextId++);
