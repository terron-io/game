...
import { recordEconomyGold } from "./EconomyLog";
...
class TradeStationStopHandler implements TrainStopHandler {
  onStop(
    ...
    if (trainOwner !== stationOwner) {
      ...
      // terron: ROI-лог — чужой поезд у вашей станции. Спека: airport.md
      recordEconomyGold(mg, stationOwner, "railExt", gold);
    }
    trainOwner.addGold(gold, station.tile());
    mg.stats().trainSelfTrade(trainOwner, gold);
    // terron: ROI-лог — self-трейд поездов = «фирменный» доход фабрики (она гоняет поезда).
    recordEconomyGold(mg, trainOwner, "factory", gold);
  }
}
...
export class Cluster {
  ...
  private isTradeStation(station: TrainStation): boolean {
    ...
    // terron: аэропорт тоже торговый узел кластера (поезда ходят к нему). Спека: airport.md
    return (
      type === UnitType.City ||
      type === UnitType.Port ||
