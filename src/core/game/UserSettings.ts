...
/** terron: раскладки прицельного управления (порядок = порядок в настройках). */
export const AIM_LAYOUTS = ["classic", "g", "flanks", "fan"] as const;
export type AimLayout = (typeof AIM_LAYOUTS)[number];
...
export class UserSettings {
  ...
  // terron: «истинные цвета скина» — raw rgb, полная непрозрачность, без тинта.
  // По умолчанию ВКЛ (исправление искажения цветов); тоггл в настройках.
  skinTrueColors() {
    return this.getBool("settings.skinTrueColors", true);
  }

  toggleSkinTrueColors() {
    this.setBool("settings.skinTrueColors", !this.skinTrueColors());
  }
  ...
  /**
   * terron: СХЕМА УПРАВЛЕНИЯ (эксперимент, см. client/hud/layers/AimControl.ts).
   * "classic" — как было всегда (радиал + меню стройки), это ДЕФОЛТ: управление
   * не должно меняться у человека само. Остальные — прицельные раскладки.
   */
  aimLayout(): AimLayout {
    const v = this.getString("settings.aimLayout", "classic");
    return (AIM_LAYOUTS as readonly string[]).includes(v)
      ? (v as AimLayout)
      : "classic";
  }

  setAimLayout(layout: AimLayout) {
    this.setString("settings.aimLayout", layout);
  }

  /** Прицельное управление включено (любая раскладка, кроме классики). */
  aimControl() {
    return this.aimLayout() !== "classic";
  }
  ...
  getFlag(): string | null {
    ...
    // terron: `clan:<tag>` — полноправный префикс клан-флага (резолвится в
    // Cosmetics.getPlayerCosmetics). НЕ мигрируем его, иначе он портится в
    // `country:clan:<tag>` и перестаёт сниматься при смене/очистке тега.
    if (
      !flag.startsWith("flag:") &&
      !flag.startsWith("country:") &&
      !flag.startsWith("clan:")
    ) {
      flag = `country:${flag}`;
  ...
  // terron: mute хранится отдельно от громкости (позиция ползунка сохраняется).
  musicMuted(): boolean {
    return this.getBool("settings.musicMuted", false);
  }
  setMusicMuted(v: boolean): void {
    this.setBool("settings.musicMuted", v);
  }
  // terron: музыка ЛОББИ — отдельный тумблер от игровой, ПО УМОЛЧАНИЮ ВЫКЛ
  // (решение владельца: базово тихо, пользователь включает сам).
  lobbyMusicMuted(): boolean {
    return this.getBool("settings.lobbyMusicMuted", true);
  }
  setLobbyMusicMuted(v: boolean): void {
    this.setBool("settings.lobbyMusicMuted", v);
  }
  soundEffectsMuted(): boolean {
    return this.getBool("settings.soundEffectsMuted", false);
  }
  setSoundEffectsMuted(v: boolean): void {
    this.setBool("settings.soundEffectsMuted", v);
  }
  ...
  soundEffectsVolume(): number {
    // terron: базовая громкость эффектов 20% (музыка остаётся 0). Применяется
    // только к новым пользователям без сохранённого значения.
    return this.getFloat("settings.soundEffectsVolume", 0.2);
  }
