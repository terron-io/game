...
// terron: ПИРАТСТВО — «Блокада»: для кораблей, которым зона блокады ЗАКРЫТА,
// путь считается по КОПИИ мини-карты, где тайлы флага помечены сушей. Кэш по
// ключу «версия воды:версия блокад:набор закрытых тайлов» — зоны меняются редко
// (появление / смерть лодки), а цепочка строится простым A* без HPA (HPA
// предсчитан под настоящую карту). Свои/союзные владельцу зоны лодки идут по
// обычной цепочке (закрытых тайлов для них нет).
// Несколько владельцев зон с разными союзами = разные наборы закрытых тайлов;
// однослотовый кэш молотил бы копию мини-карты на каждый новый корабль.
const BLOCKED_CHAIN_CACHE_MAX = 8;
const _blockedChainCache = new WeakMap<
  Game,
  Map<string, PathFinder<TileRef>>
>();

/**
 * ⚠️ terron 23.08 — БЛОКАДА БОЛЬШЕ НЕ ТРОГАЕТ ПОИСК ПУТИ (решение владельца:
 * «убрать эффект… чтобы не влияло, просто топить лодки — и всё»).
 *
 * Здесь была ТЕНЕВАЯ КАРТА: зона превращалась в «сушу», и вокруг неё строились
 * объезды. Красиво, но цена запредельная: копия мини-карты и полная пересборка
 * A*-цепочки СВОЯ У КАЖДОГО игрока, к которому зона применяется, плюс сброс
 * кэша пути у КАЖДОЙ лодки на карте при любом изменении зоны. Флотилия
 * подходит по одной лодке, зон бывает несколько — симуляция вставала колом
 * («сим 2.0/10», «всё зависло»), и лечению троттлингом это поддавалось плохо.
 *
 * Теперь зона — не препятствие, а ОПАСНОСТЬ: пути её не замечают, а чужие
 * корабли, оказавшиеся внутри, топит сама блокада (BlockadeExecution).
 * Стоимость для симуляции — ноль.
 */
...
export class WaterPathFinder implements SteppingPathFinder<TileRef> {
  ...
  constructor(
    ...
    // terron 23.08: владелец кораблю больше не нужен для путей (блокада не
    // меняет проходимость). Параметр оставлен: он есть у всех вызовов, а
    // выкидывать его = трогать десяток экзекуций ради нуля пользы.
