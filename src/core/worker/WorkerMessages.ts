...
export interface InitializedMessage extends BaseWorkerMessage {
  type: "initialized";
  // terron: если init воркера упал (напр. карта не загрузилась офлайн) — кладём
  // сюда текст ошибки, чтобы главный поток показал её, а не немой 60с-таймаут.
  error?: string;
  // terron 20.07: разбивка инициализации ВНУТРИ воркера (мс). Телеметрия
  // показала, что весь экран загрузки — это ожидание первого тика (медиана ~1с,
  // p90 ~2.8с, на холодную до 6с), но что именно там долго — карта, нации или
  // сборка мира — видно не было. Уезжает в load_trace.
