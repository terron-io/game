...
export class SpawnExecution implements Execution {
  ...
  constructor(
    ...
    public tile?: TileRef,
    // terron: форс-спавн (авто-размещение по истечении грейса) — игнорирует лимит
    // времени грейса, но НЕ ре-спавнит уже заспавнившегося.
  ...
  tick(ticks: number) {
    ...
    // terron: грейс после старта — если фаза спавна уже закончилась, заспавниться
    // можно ещё TERRON_SPAWN_GRACE_SECONDS секунд, и только тем, кто ещё НЕ заспавнился
    // (никаких ре-роллов после старта). Дальше окна — отказ.
    if (!this.mg.inSpawnPhase()) {
      // Уже заспавнился — никогда не ре-роллим после старта (даже форсом).
      if (player.hasSpawned()) {
        return;
      }
      // Окно грейса истекло — обычный спавн отказывает; форс-авто-спавн проходит.
      if (
        !this.forced &&
        this.mg.elapsedGameSeconds() > TERRON_SPAWN_GRACE_SECONDS
      ) {
        return;
      }
    }
