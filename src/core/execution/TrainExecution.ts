...
export class TrainExecution implements Execution {
  ...
  init(mg: Game, ticks: number): void {
    this.mg = mg;
    // terron: ТОПЛИВО — вагоны владельца едут быстрее. FUEL.md
    this.speed = this.speed * fuelSpeedMult(this.owner());
    // terron: ВЗРЫВНЫЕ ПОЕЗДА — пассив депо: составы владельца вдвое быстрее
    // (больше рейсов = больше денег). Множители Топлива и депо перемножаются
    // осознанно: это две разные ульты, вместе их не собрать. TRAINS.md
