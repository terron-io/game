...
export class NationNukeBehavior {
  ...
  // «пара ядерок» от соседей. new-units/NEBO.md
  private readonly counteredOurSkyIds = new Set<number>();
  ...
  maybeCounterOurSky() {
    ...
    for (const hq of this.game.units(UnitType.SatelliteStrike)) {
      if (!hq.isActive() || !hq.isUnderConstruction()) continue;
      if (this.counteredOurSkyIds.has(hq.id())) continue;
      const owner = hq.owner();
      if (
        owner === this.player ||
        owner.type() === PlayerType.Bot ||
        this.player.isFriendly(owner) ||
        this.attackBehavior.shouldAttack(owner) === false
      ) {
        continue;
      }

      // Выбор бомбы по карману (как в maybeSendNuke), без хитрого скоринга —
      // цель одна и известна (тайл комплекса).
      let nukeType: UnitType.AtomBomb | UnitType.HydrogenBomb | null = null;
      if (
        !config.isUnitDisabled(UnitType.HydrogenBomb) &&
      ...
      if (nukeType === null) continue;

      const tile = hq.tile();
      if (this.player.canBuild(nukeType, tile) === false) continue;

      this.counteredOurSkyIds.add(hq.id());
      this.sendNuke(tile, nukeType, owner);
      return; // один комплекс за проход
    }
  }
