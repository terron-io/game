import { TERRON_PEACE_NATION_ALLIES } from "../../configuration/TerronTuning";
import {
  Difficulty,
  Game,
  GameMode,
...
export class NationAllianceBehavior {
  ...
  handleAllianceRequests() {
    ...
    for (const req of this.player.incomingAllianceRequests()) {
      ...
      // terron: ДВОРЕЦ НАЦИЙ — нации не отказывают владельцу, пока у него
      // меньше TERRON_PEACE_NATION_ALLIES нацио-союзов (PEACE.md, пассив 2).
      if (
        req.requestor().hasUltimate(UnitType.PeacePalace) &&
        req
          .requestor()
          .alliances()
          .filter((a) => a.other(req.requestor()).type() === PlayerType.Nation)
          .length < TERRON_PEACE_NATION_ALLIES
      ) {
        req.accept();
        continue;
      }
