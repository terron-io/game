...
export class NationMIRVBehavior {
  ...
  considerMIRV(): boolean {
    ...
    // terron: МИРВ теперь разблокируется зданием «Ядерный завод» (выбор ульты
    // бота). Нет завода — строим его сначала; прочие ульты бот пока НЕ использует.
    // Спека: new-units/ULTIMATES.md
    if (!this.player.hasUltimate(UnitType.NuclearFactory)) {
      return this.maybeBuildNuclearFactory();
    }
  ...
  // terron: бот строит «Ядерный завод» (разблокировка МИРВа). Одно здание на
  // игрока; ждём достройки, если уже строится. Прочие ульты бот не выбирает.
  private maybeBuildNuclearFactory(): boolean {
    if (this.player === null) throw new Error("not initialized");
    // Уже есть/строится завод — ждём достройки (hasUltimate станет true).
    if (this.player.units(UnitType.NuclearFactory).length > 0) return false;
    // Выбор ульты уже занят чем-то другим (не должно быть у бота, но гард).
    const choice = this.player.ultimateChoice();
    if (choice !== null && choice !== UnitType.NuclearFactory) return false;
    if (this.player.gold() < this.cost(UnitType.NuclearFactory)) return false;
    const tiles = randTerritoryTileArray(this.random, this.game, this.player, 25);
    for (const t of tiles) {
      if (this.player.canBuild(UnitType.NuclearFactory, t) === false) continue;
      this.game.addExecution(
        new ConstructionExecution(this.player, UnitType.NuclearFactory, t),
      );
      return true;
    }
    return false;
