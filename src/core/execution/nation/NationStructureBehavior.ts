...
function getStructureRatios(
  ...
  return {
    ...
    // terron: авиация — боты строят аэропорты. Реже ж/д-строек (дорогой ×2 порт), но нужно
    // ≥2 для авиа-экономики. 0.34 → ~2 аэропорта на 6 городов. Спека: airport.md
    [UnitType.Airport]: {
      ratioPerCity: 0.34,
      perceivedCostIncreasePerOwned: 1,
    },
...
export class NationStructureBehavior {
  ...
  private doHandleStructures(): boolean {
    ...
    const buildOrder: UnitType[] = [
      UnitType.Port,
      UnitType.Factory,
      UnitType.Airport, // terron: авиация — после порта/фабрики. Спека: airport.md
      UnitType.SAMLauncher,
      UnitType.MissileSilo,
    ];
  ...
  /**
   * terron: авиация — value-функция аэропорта. Хочет: (1) быть подальше от других своих
   * аэропортов (борта летают между кластерами — длинные прибыльные рейсы + мост между
   * скоплениями), (2) быть рядом с фабрикой (иначе не подключится к ж/д кластеру). Спека: airport.md
   */
  private airportValue(): (tile: TileRef) => number {
    const game = this.game;
    const otherAirports = this.player.units(UnitType.Airport);
    const factoryTiles = new Set(
      this.player.units(UnitType.Factory).map((u) => u.tile()),
    );
    const railRange = game.config().trainStationMaxRange();

    return (tile) => {
      let w = 0;

      // Prefer to be as far as possible from other airports (spread across clusters).
      ...
      // Bonus for sitting inside a rail cluster (near a factory → gets a station).
      if (factoryTiles.size > 0) {
        const [, closestFactoryDist] = closestTile(game, factoryTiles, tile);
        if (closestFactoryDist <= railRange) {
          w += railRange - closestFactoryDist;
        }
      }
