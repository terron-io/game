...
export class AiAttackBehavior {
  ...
  // буфером; сброс, когда входящих атак нет. new-units/NEBO.md
  private retaliationArmedTick: number | null = null;

  /**
   * terron: «Небо наше» — во время блэкаута бот НЕ расширяется (нет захвата
   * terraNullius/соседей), но ОТВЕЧАЕТ тому, кто на него напал: контр-атака
   * войсками против крупнейшего входящего атакующего. Авто-оборона территории
   * (сопротивление войск на своей земле) работает и без этого — в симуляции.
   *
   * `bufferTicks > 0` — БУФЕР РЕАКЦИИ: с момента, как бот заметил атаку, он
   * выжидает bufferTicks и только потом отвечает («узнаёт» о нападении с
   * задержкой). Переиспользуемо для будущих ульт скрытной агрессии.
   * new-units/NEBO.md
   */
  maybeRetaliate(bufferTicks = 0): void {
    if (this.player === null) throw new Error("not initialized");
    const attacker = this.findIncomingAttackPlayer();
    if (attacker === null) {
      // Атак нет — сбрасываем «часы» буфера до следующего нападения.
      this.retaliationArmedTick = null;
      return;
    }
    if (bufferTicks > 0) {
      this.retaliationArmedTick ??= this.game.ticks();
      if (this.game.ticks() - this.retaliationArmedTick < bufferTicks) {
        return; // буфер ещё не истёк — отвечаем позже
      }
    }
  ...
  private findNearestIslandEnemy(): Player | null {
    ...
    // Check if we have any shore tiles to launch from
    // terron ПЕРФ (15.08): свои береговые тайлы — ИНВАРИАНТ этого вызова, а
    // строились заново на КАЖДОГО кандидата в цикле ниже (Array.from + filter
    // по всей границе × число кандидатов). Считаем один раз; заодно он же
    // отвечает на hasShore. Содержимое и порядок те же — closestTwoTiles
    // получает идентичный вход, детерминизм цел.
    const myShores = Array.from(this.player.borderTiles()).filter((t) =>
      this.game.isShore(t),
    );
    if (myShores.length === 0) return null;
  ...
  private sendLandAttack(target: Player | TerraNullius): boolean {
    ...
    if (target.isPlayer() && this.player.type() === PlayerType.Nation) {
      // terron: эмодзи-«трэштолк» — только у настоящих наций (NationExecution). Нация-
      // сепаратист Раскола идёт через TribeExecution без emojiBehavior — просто без эмодзи.
  ...
  private sendBoatAttack(target: Player): boolean {
    ...
    if (target.isPlayer() && this.player.type() === PlayerType.Nation) {
      // terron: см. sendLandAttack — эмодзи опциональны (сепаратист Раскола без них).
      this.emojiBehavior?.maybeSendAttackEmoji(target);
    }
