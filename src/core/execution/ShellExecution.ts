import { TERRON_PIRACY_DAMAGE_MULT } from "../configuration/TerronTuning";
...
export class ShellExecution implements Execution {
  ...
  private effectOnTarget(): number {
    ...
    // terron: ПИРАТСТВО — пиратская лодка бьёт втрое слабее (TerronTuning).
    const pirate = this._owner.hasUltimate(UnitType.Piracy)
      ? TERRON_PIRACY_DAMAGE_MULT
      : 1;
    return Math.round((baseDamage / 250) * damageMultiplier * pirate);
  }
