...
export class NationExecution implements Execution {
  ...
  private airBehavior!: NationAirBehavior; // terron: авиация
  private mg: Game;
  ...
  tick(ticks: number) {
    ...
    // terron: «Небо наше» — пока действует блэкаут спутников, ослеплённый бот
    // (все, кроме владельца и его команды) НЕ ведёт наступление: не расширяется
    // (экспансия/захват), не кидает ракеты (ядерки/МИРВ), не шлёт десант/дроны.
    // Всё остальное продолжает: стройку, корабли, дипломатию/торговлю, эмодзи,
    // авто-оборону территории и ОТВЕТ тому, кто на него напал. Порядок вызовов
    // сохранён как в апстриме — расходятся только атакующие. new-units/NEBO.md
    const blinded = this.mg.satelliteBlackoutBlinds(this.player);
    ...
    if (!blinded) this.mirvBehavior.considerMIRV();
    this.structureBehavior.handleStructures();
    this.warshipBehavior.maybeSpawnWarship();
    this.handleEmbargoesToHostileNations();
    // Экспансия заморожена под блэкаутом — только контр-атака напавшему, и та с
    // буфером реакции (бот «узнаёт» об атаке с задержкой TERRON_OURSKY_REACTION_
    // BUFFER_TICKS). new-units/NEBO.md
    if (blinded) {
      this.attackBehavior.maybeRetaliate(TERRON_OURSKY_REACTION_BUFFER_TICKS);
    } else {
      this.attackBehavior.maybeAttack();
    }
    this.warshipBehavior.counterWarshipInfestation();
    if (!blinded) {
      // terron: «Небо наше» — сбить вражеский антиспутник, пока он строится.
      this.nukeBehavior.maybeCounterOurSky();
      this.nukeBehavior.maybeSendNuke();
      // terron: авиация — нации применяют десант и дроны с аэропортов
      this.airBehavior.maybeAirAssault();
      this.airBehavior.maybeSendDrone();
    }
  }
  ...
  private initializeBehaviors(): void {
    ...
    // terron: авиация — десант + дроны с аэропортов
    this.airBehavior = new NationAirBehavior(
      this.random,
      this.mg,
      this.player,
      this.attackBehavior,
