...
export class TribeExecution implements Execution {
  ...
  tick(ticks: number) {
    ...
    if (this.attackBehavior === null) {
      ...
      // Send an attack on the first tick — но не под блэкаутом «Небо наше»
      // (ослеплённое племя не расширяется даже на первом ходу).
      if (!this.mg.satelliteBlackoutBlinds(this.tribe)) {
    ...
    // terron: «Небо наше» — ослеплённое племя (все, кроме владельца ульты и его
    // команды) НЕ ведёт наступление: не расширяется, не захватывает соседей/
    // terraNullius. Только контр-атака напавшему с буфером реакции — как у наций
    // (NationExecution). Снятие блэкаута → обычная агрессия. new-units/NEBO.md
    if (this.mg.satelliteBlackoutBlinds(this.tribe)) {
