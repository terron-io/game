...
export class AttackExecution implements Execution {
  ...
  tick(ticks: number) {
    ...
    if (targetPlayer && this._owner.isFriendly(targetPlayer)) {
      ...
      // terron: ДВОРЕЦ НАЦИЙ — НАВЯЗАННЫЙ пакт не отзывает атаки, начатые до
      // него (канон PEACE.md: «добивающий удар нельзя украсть кнопкой»).
      const pact = this._owner.pactWith(targetPlayer);
      if (pact === null || this._startedAt >= pact) {
        this.retreat();
        return;
      }
    }
    ...
    while (numTilesPerTick > 0) {
      ...
      if (this.toConquer.size() === 0) {
        // terron ПЕРФ (15.08): здесь стоял refreshToConquer() — ПОЛНАЯ пересборка
        // очереди обходом ВСЕЙ границы владельца (все фронты: океан, союзники,
        // чужие) — результат которой никем не читался: retreat() строкой ниже
        // безусловно делает attack.delete() и active=false, экзекуция мертва.
        // Так завершается БОЛЬШИНСТВО атак (экспансия в пустошь, доедание
        // ботов); у поздней империи граница — десятки тысяч тайлов, у наций
        // Impossible до 100 параллельных атак. Чистая мёртвая работа.
        // Детерминизм цел: PRNG у экзекуции свой (не глобальный) и после
        // смерти не используется; мутации attack-границы уничтожает delete().
        this.retreat();
        return;
      }
